...
router.get('/doctors', auth, async (req, res) => {
  ...
  try {
    ...
    const doctorDetails = await Doctor.find({ userId: { $in: doctors.map((d) => d._id) } });
    ...
    if (!availableTimeSlots.length) {
      ...
      return res.status(404).json({ msg: 'No doctors with available time slots' });
    }

    const doctorsWithSlots = doctorDetails.filter((d) =>
      availableTimeSlots.some((t) => t.doctorId.toString() === d.userId.toString())
    );
    ...
    const result = doctorsWithSlots.map((d) => ({
      ...d._doc,
      name: doctors.find((u) => u._id.toString() === d.userId.toString())?.name || 'Unknown',
      email: doctors.find((u) => u._id.toString() === d.userId.toString())?.email || 'Unknown',
    }));
...
router.post('/book', auth, async (req, res) => {
  ...
  try {
    const { date, doctorId, doctorName, timeSlotId } = req.body;

    // Validate request body
    if (!date || !doctorId || !doctorName || !timeSlotId) {
      return res.status(400).json({ msg: 'date, doctorId, doctorName, and timeSlotId are required' });
    }

    // Find the time slot and validate it
    const timeSlot = await TimeSlot.findById(timeSlotId);
    if (!timeSlot) return res.status(404).json({ msg: 'Time slot not found' });
    if (timeSlot.isBooked) return res.status(400).json({ msg: 'Time slot already booked' });
    if (timeSlot.doctorId.toString() !== doctorId) {
      return res.status(400).json({ msg: 'Time slot does not belong to the specified doctor' });
    }
    if (timeSlot.date !== date) {
      return res.status(400).json({ msg: 'Date mismatch with time slot' });
    }
    ...
    if (!user || user.role !== 'doctor' || !user.isApproved || user.name !== doctorName) {
      return res.status(400).json({ msg: 'Invalid doctor details' });
    }
    ...
    const appointment = new Appointment({
      ...
      doctorId: doctor._id, // Use Doctor model's _id
      doctorName,
      date,
      timeSlot: `${timeSlot.startTime} - ${timeSlot.endTime}`, // Construct timeSlot
      timeSlotId,
      status: 'pending', // Changed to 'pending' to match schema
    });
...
router.get('/timeslots/:doctorId', auth, async (req, res) => {
  ...
  try {
    ...
    console.log(`Found ${timeSlots.length} available time slots for doctor ID: ${doctorId}`);
    res.json(timeSlots);
  } catch (err) {
